<x-layouts.app>
    ...
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100">{{ __('Edit Sub Category') }}</h1>
        <p class="text-gray-600 dark:text-gray-400 mt-1">
            {{ __('Update sub category details') }}
        </p>
    </div>
    ...
    <div class="p-6">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            ...
            <form action="{{ route('subcategories.update', $subcategory) }}" method="POST" class="max-w-2xl space-y-4">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <select name="category_id"
                        ...
                        @foreach($categories as $c)
                            <option value="{{ $c->id }}"
                                @selected(old('category_id', $subcategory->category_id) == $c->id)>
                                {{ $c->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="flex items-center gap-3 pt-4">
                    <x-button type="primary">{{ __('Update Sub Category') }}</x-button>
                    <a href="{{ route('subcategories.index') }}"
