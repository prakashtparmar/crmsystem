    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-5 flex items-center text-xs text-gray-500 dark:text-gray-400">
            ...
            <a href="{{ route('product-attributes.index') }}"
                class="text-blue-600 dark:text-blue-400 hover:underline">{{ __('Product Attributes') }}</a>
            <span class="mx-2">›</span>
        ...
        <!-- Header -->
        <div class="mb-6">
            <h1 class="text-xl font-semibold text-gray-900 dark:text-gray-100">{{ __('Edit Attribute') }}</h1>
            <p class="text-sm text-gray-600 dark:text-gray-400">{{ __('Update product attribute') }}</p>
        </div>
        ...
        <div class="max-w-5xl">
            <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
                ...
                <form action="{{ route('product-attributes.update', $attribute) }}" method="POST" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="p-5 border-b border-gray-200 dark:border-gray-700">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-3xl">
                                ...
                                <select name="product_id"
                                    ...
                                    @foreach ($products as $product)
                                        <option value="{{ $product->id }}"
                                            @selected(old('product_id', $attribute->product_id) == $product->id)>
                                            {{ $product->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <x-forms.input label="Attribute Name" name="name"
                                value="{{ old('name', $attribute->name) }}" required />

                            <x-forms.input label="Attribute Value" name="value"
                                value="{{ old('value', $attribute->value) }}" required />
                        </div>
                    </div>
