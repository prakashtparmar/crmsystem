<x-layouts.app>
    ...
    <div class="mb-6 flex items-center text-sm">
        ...
        <span class="text-gray-500 dark:text-gray-400">{{ __('Product Attributes') }}</span>
    </div>

    <!-- Header -->
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100">{{ __('Product Attributes') }}</h1>
            <p class="text-gray-600 dark:text-gray-400 mt-1">
                {{ __('Manage all product attributes') }}
            </p>
        </div>
    ...
    <div class="p-6">
        <div
            ...
            <table id="attributesTable" class="w-full text-xs border-collapse">
                    ...
                    @foreach ($attributes as $attribute)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/40">
                            <td class="px-3 py-2 text-center">
                                <input type="checkbox" class="row-checkbox" value="{{ $attribute->id }}">
                            </td>

                            <td class="px-3 py-2 text-gray-500">{{ $attribute->id }}</td>
                            <td class="px-3 py-2">{{ $attribute->product_id }}</td>
                            <td class="px-3 py-2 font-medium">
                                {{ $attribute->product?->name ?? '—' }}
                            </td>
                            <td class="px-3 py-2">{{ $attribute->name }}</td>
                            <td class="px-3 py-2">{{ $attribute->value }}</td>
                            ...
                            <td class="px-3 py-2 text-right relative">
                                <div class="relative inline-block text-left" x-data="{ open: false }">
                                    ...
                                    <div x-cloak x-show="open" @click.away="open = false" x-transition
                                        ...
                                        <form action="{{ route('product-attributes.destroy', $attribute) }}" method="POST"
