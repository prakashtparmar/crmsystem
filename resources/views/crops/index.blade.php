<x-layouts.app>
    ...
    <div class="mb-6 flex items-center text-sm">
        ...
        <span class="text-gray-500 dark:text-gray-400">{{ __('Crops') }}</span>
    </div>

    <!-- Header -->
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100">{{ __('Crops') }}</h1>
    ...
    <div class="p-6">
        <div
            ...
            <table id="cropsTable" class="w-full text-sm">
                    ...
                    @foreach ($crops as $crop)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/40">
                            <td class="px-3 py-2 text-center">
                                <input type="checkbox" class="row-checkbox" value="{{ $crop->id }}">
                            </td>

                            <td class="px-3 py-2 text-gray-500">{{ $crop->id }}</td>

                            <td class="px-3 py-2 font-medium text-gray-800 dark:text-gray-100">
                                {{ $crop->name }}
                            </td>

                            <td class="px-3 py-2 text-gray-500">{{ $crop->slug }}</td>

                            <td class="px-3 py-2">
                                <span
                                    class="px-2 py-1 text-xs rounded-full
                                    {{ $crop->is_active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
