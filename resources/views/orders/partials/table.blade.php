    ...
    <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm p-4">
        ...
        <table id="ordersTable" class="w-full text-xs min-w-max border-collapse">
            <thead class="bg-gray-50 dark:bg-gray-700/50 text-gray-600 dark:text-gray-300">
                <tr>
                    @foreach ([
                        '', 'ID', 'UUID', 'Order Code', 'Customer', 'Email', 'Phone', 'Order Date',
                        'Created At', 'Updated At', 'Expected', 'Sub Total', 'Tax', 'Shipping',
                        'Discount', 'Grand Total', 'Status', 'Payment', 'Method', 'Transaction',
                        'Paid At', 'Completed', 'Customer ID', 'User ID', 'Billing Address',
                        'Shipping Address', 'Meta', 'Created By', 'Updated By', 'Deleted At',
                        'Confirmed At', 'Cancelled At', 'Invoice #', 'Invoice Status',
                        'Shipment Status', 'Carrier', 'Tracking', 'Shipped At', 'Delivered At',
                        'Total Paid', 'Balance', 'Action'
                    ] as $h)
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                @foreach ($orders as $order)
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/40 align-top">
                        ...
                        <td class="px-3 py-2 text-gray-500">{{ $order->expected_delivery_at?->format('d M Y') ?? '—' }}</td>
                        ...
                        <td class="px-3 py-2 text-gray-500">{{ $order->confirmed_at?->format('d M Y H:i') ?? '—' }}</td>
                        <td class="px-3 py-2 text-gray-500">{{ $order->cancelled_at?->format('d M Y H:i') ?? '—' }}</td>
                        ...
                        <td class="px-3 py-2">{{ $order->shipments->first()?->shipped_at?->format('d M Y') ?? '—' }}</td>
                        <td class="px-3 py-2">{{ $order->shipments->first()?->delivered_at?->format('d M Y') ?? '—' }}</td>
