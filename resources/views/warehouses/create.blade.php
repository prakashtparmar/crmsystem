    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-5 flex items-center text-xs text-gray-500 dark:text-gray-400">
            ...
            <a href="{{ route('warehouses.index') }}"
               class="text-blue-600 dark:text-blue-400 hover:underline">{{ __('Warehouses') }}</a>
            <span class="mx-2">›</span>
        ...
        <!-- Header -->
        <div class="mb-6">
            <h1 class="text-xl font-semibold text-gray-900 dark:text-gray-100">{{ __('Create Warehouse') }}</h1>
            <p class="text-sm text-gray-600 dark:text-gray-400">{{ __('Add a new warehouse location') }}</p>
        </div>
                ...
                <form action="{{ route('warehouses.store') }}" method="POST" class="space-y-6">
                    ...
                    <div class="p-5 border-b border-gray-200 dark:border-gray-700">
                        ...
                        <div class="grid grid-cols-1 gap-4 max-w-3xl">
                            <x-forms.input label="Warehouse Name" name="name" value="{{ old('name') }}" required />
                            <x-forms.input label="Code" name="code" placeholder="WH-001" value="{{ old('code') }}" required />

                            <!-- Address (no component to avoid error) -->
                            <div class="flex flex-col">
                                <label class="block text-sm font-medium mb-1">Address</label>
                                <textarea name="address" rows="3"
                                    class="w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900">{{ old('address') }}</textarea>
                            </div>
                        </div>
                    </div>
