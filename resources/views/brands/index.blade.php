<x-layouts.app>
    ...
    <div class="mb-6 flex items-center text-sm">
        ...
        <span class="text-gray-500 dark:text-gray-400">{{ __('Brands') }}</span>
    </div>

    <!-- Header -->
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100">{{ __('Brands') }}</h1>
            <p class="text-gray-600 dark:text-gray-400 mt-1">
                {{ __('Manage all product brands') }}
            </p>
        </div>
    ...
    <div class="p-6">
        <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm p-4 overflow-visible">
            <table id="brandsTable" class="w-full text-sm whitespace-nowrap">
                    ...
                    @foreach ($brands as $brand)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/40">
                            <td class="px-3 py-2 text-center">
                                <input type="checkbox" class="row-checkbox" value="{{ $brand->id }}">
                            </td>

                            <td class="px-3 py-2 text-gray-500">{{ $brand->id }}</td>
                            ...
                            <td class="px-3 py-2">
                                <span class="px-2 py-1 text-xs rounded-full {{ $brand->is_active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                window.initDataTable('#brandsTable');
            });
        </script>
    @endpush
</x-layouts.app>
